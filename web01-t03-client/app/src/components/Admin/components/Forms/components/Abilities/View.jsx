...
const View = ( { target: { id }, abilities, abilitie: { title, description }, submit, remove, edit, onChange, classes } ) => {

  return (
    <div>
      <div className={ classes.panel }>
        {
          abilities.map( ( abilitie, key ) => {
            return (
              <ListItem key={ key } id={ abilitie._id } title={ abilitie.title } remove={ remove } edit={ edit }>
                <Typography component="p" variant="body1"><strong>Descrição: </strong></Typography>
                <Typography component="p" variant="body1">{ abilitie.description }</Typography>
              </ListItem>
            )
          } )
        }
